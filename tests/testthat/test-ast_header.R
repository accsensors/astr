...
test_that("fread_ast_header works with standard rev210 UPASv2x file", {
  filename <- 'PSP01066_LOG_2026-05-19T16_22_25UTC_standard30s-210_----------.txt'
  file <- system.file("extdata", filename, package = "astr", mustWork = TRUE)
  header_raw <- fread_ast_header(file)
  expect_identical(tail(header_raw$header, n=1)[[1]], "SAMPLE LOG")
  expect_identical(header_raw$diag, NULL)
...
test_that("fread_ast_header works with rev 210 standard DIAGNOSTIC UPASv2x file", {
  filename <- 'PSP01066_LOG_2026-05-19T16_35_06UTC_DIAGNOSTIC-----___________.txt'
  file <- system.file("extdata", filename, package = "astr", mustWork = TRUE)
  header_raw <- fread_ast_header(file)
  expect_identical(tail(header_raw$header, n=1)[[1]], "DIAGNOSTIC TEST")
  expect_identical(tail(header_raw$diag, n=1)[[1]], "SAMPLE LOG")
...
test_that("transpose_ast_header works with standard rev 210 UPASv2x file", {
  filename <- 'PSP01066_LOG_2026-05-19T16_22_25UTC_standard30s-210_----------.txt'
  file <- system.file("extdata", filename, package = "astr", mustWork = TRUE)
  header_raw  <- fread_ast_header(file)
  header_wide <- transpose_ast_header(header_raw$header, diag = header_raw$diag)
...
test_that("transpose_ast_header works with rev 210 standard DIAGNOSTIC UPASv2x file", {
  filename <- 'PSP01066_LOG_2026-05-19T16_35_06UTC_DIAGNOSTIC-----___________.txt'
  file <- system.file("extdata", filename, package = "astr", mustWork = TRUE)
  header_raw  <- fread_ast_header(file)
  header_wide <- transpose_ast_header(header_raw$header, diag = header_raw$diag)
...
test_that("Variables in UPASv2x log file headers are being formatted as the correct type", {
  filename <- 'PSP01066_LOG_2025-03-06T19_42_26UTC_standard30s_____----------.txt'
  file   <- system.file("extdata", filename, package = "astr", mustWork = TRUE)
...
test_that("UPASv2x log file headers written using firmware rev 200, rev210, and rev < 200 can be combined when update_names = TRUE", {
  ...
  fname_210 <- 'PSP01066_LOG_2026-05-19T16_22_25UTC_standard30s-210_----------.txt'
  ...
  file_210  <- system.file("extdata", fname_210, package = "astr", mustWork = T)
  header_157 <- read_ast_header(file_157, update_names = T)
  header_200 <- read_ast_header(file_200)
  header_210 <- read_ast_header(file_210)
  expect_no_error(dplyr::bind_rows(header_200, header_210, header_157))
  expect_equal(ncol(header_200), ncol(dplyr::bind_rows(header_200, header_210, header_157)))
  expect_identical(colnames(header_157),
                   colnames(dplyr::select(header_200,
                                          dplyr::any_of(colnames(header_157)))))
  expect_identical(colnames(dplyr::select(header_157,
                                          !(PercentTimeWorn))),
                   colnames(dplyr::select(header_210,
                                          dplyr::any_of(colnames(header_157)))))
})
...
test_that("read_ast_header works with all UPASv2x firmwares", {
  ...
  expect_snapshot(read_ast_header(upasv2x_rev210_file, update_names=FALSE))
  ...
  expect_snapshot(read_ast_header(upasv2x_rev210_diag_file, update_names=FALSE))
})
