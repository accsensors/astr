...
You can use either the `lapply` function or the `map` function to read header data from multiple log files and combine those data into a single data frame with one row for each file. 
...
```{r header_map}
data_upas_h <- list.files(path = ".", pattern = "^PS.*.txt$", full.names = T) %>%
  purrr::map(read_ast_header, update_names = T) %>%
  dplyr::bind_rows()
```

You can also use either the `lapply` function or the `map` function to read sample logs from multiple files and combine those data into a single data frame.  A column with the filename will be appended so that the data from each individual file can be identified easily.  
...
```{r log_map}
data_upas <- list.files(path = ".", pattern = "^PS.*.txt$", full.names = T) %>%
  purrr::map(read_ast_log, update_names = T) %>%
  dplyr::bind_rows()
```
...
```{r example_map}
multiple_upas_logs <- system.file("extdata", package = "astr", mustWork = T) |>
    list.files(pattern = "^PS.*.txt$", full.names = T) %>%
    lapply(read_ast_log, update_names=T) %>%
    dplyr::bind_rows()
